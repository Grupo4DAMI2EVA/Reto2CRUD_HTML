...
let selectedItemMessage, quantityDisplay, decreaseBtn, increaseBtn, deleteSelectedBtn;
...
async function init() {
  ...
  deleteSelectedBtn = document.querySelector('[data-role="selected-delete-btn"]');
...
async function changeSelectedQty(delta) {
  console.log('Cambiando cantidad, delta:', delta, 'selectedItemId:', selectedItemId);
  ...
  console.log(`Cambiando cantidad de ${currentQty} a ${newQty} para item ${item.name}`);
...
async function onBuy() {
  ...
  try {
    ...
    if (!resp.ok || json.error || !json.success) {
      let errorMsg = json.error || json.message || "Error al procesar la compra";
